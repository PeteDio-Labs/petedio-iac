# ⛔ NOT DECLARED — the container does not exist and cannot be recreated as-is.
#
# registry-106 died with pve01 on 2026-09-03. Its config maps uid/gid 200
# straight through for zot (`lxc.idmap`), and `pct restore` fails on that with
# "failed to switch to gid 0". Restoring with --ignore-unpack-errors "succeeds"
# and produces a container with no /etc/os-release — a broken filesystem that
# looks restored.
#
# It also declared `sdb3-storage`, which was pve01's and is gone, so a create
# would fail even if the idmap were solved.
#
# The module below is COMMENTED OUT rather than deleted, because it is the
# record of how this was built and what it needs. To bring zot back: solve the
# idmap first, then uncomment, then set a datastore that exists.
#
# Archive: `pvesm list ollama-backups | grep vzdump-lxc-106-`
# docker.pdlab.dev is down until this is resolved.
