#!/usr/bin/env bash
# lxc-features-235.sh — set the container `features` (nesting + keyctl) on the
# Plane LXC 235 out-of-band, on the Proxmox node, over SSH-as-root. (PET-86)
#
...
#
# NB: Plane needs no tun device (no CNI, no function networking) — it is plain Docker
# Compose. The commented TUN line below is inherited from the faasd script; leave it off.
#
# SAFE: it touches ONLY container 235, and only reboots 235 (which is empty until
# the configure-openfaas rollout). Idempotent — if the features are already present
# it makes no change and does NOT reboot.
#
#   pve host:  $PVE_HOST  (default 192.168.50.10 = pve01, where 235 lives)
#   pve key :  $PVE_SSH_KEY (default ~/.ssh/id_ed25519_proxmox_pedro — bare-metal root)
#   lxc key :  $LXC_SSH_KEY (default ~/.ssh/id_ed25519_ansible — the key TF installs in 235)
# NB: the Proxmox NODE and the LXC use DIFFERENT keys — the node is bare metal (root via
# the proxmox key), 235 is a TF-created LXC (root via the ansible key). Don't conflate them.
set -euo pipefail

PVE_HOST="${PVE_HOST:-192.168.50.10}"
PVE_SSH_USER="${PVE_SSH_USER:-root}"
VMID="${VMID:-235}"
LXC_IP="${LXC_IP:-192.168.50.235}"
...
# If Plane CNI/function networking fails to come up, a tun device is usually the
...
step "Rebooting LXC $VMID so the new features take effect (235 is empty — safe)"
