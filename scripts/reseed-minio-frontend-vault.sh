#!/usr/bin/env bash
# reseed-minio-frontend-vault.sh — (re)seed kv/services/minio-frontend with a MinIO
# service account scoped to JUST the `co-latro-frontend` bucket, so the poker-api
# rollout (configure-poker-api.yml) can `mc mirror` the frontend dist onto LXC 230.
#
# WHY A SEPARATE CREDENTIAL: kv/iac/minio is scoped to the `tfstate` bucket ONLY
# (see reseed-minio-vault.sh) — it can't read co-latro-frontend. And the rollout
# runs under the `ansible` Vault policy, which reads kv/services/* (not kv/iac/* for
# app data). So we mint a least-privilege, frontend-bucket-only svcacct and store it
# under kv/services/minio-frontend (readable by the ansible policy).
#
...
POLICY="$MCCFG/frontend-policy.json"
cat > "$POLICY" <<JSON
...
step "Writing the new creds to Vault kv/services/minio-frontend"
vault kv put kv/services/minio-frontend \
  access_key="$NEW_AK" secret_key="$NEW_SK" endpoint="$MINIO_ENDPOINT" bucket="$BUCKET" >/dev/null
echo "kv/services/minio-frontend updated (readable by the ansible policy)."

step "Done"
echo "The poker-api rollout can now mc-mirror $BUCKET onto /var/www/co-latro."
